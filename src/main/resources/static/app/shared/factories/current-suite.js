(function(){
	...
	function currentSuite (){
		...
		function setUpSpecObject(newSpecs){
			...
			if(this.currentSpecObject.length !== 0){
			
				browserLength = this.currentSpecObject.browsers.length;
				platformLength = this.currentSpecObject.platforms.length;
				
				for(i = 0; i < browserLength; i++){
					if(this.currentSpecObject.browsers[i].chosen === true){
						setChosenBrowserById(this.currentSpecObject.browsers[i].browserid, newSpecs);
					}
				}
				
				for(i = 0; i < platformLength; i++){
					if(this.currentSpecObject.platforms[i].chosen){
						setChosenPlatformByname(this.currentSpecObject.platforms[i].osname, newSpecs);
					}
				}
			}
			
			this.currentSpecObject = newSpecs;
