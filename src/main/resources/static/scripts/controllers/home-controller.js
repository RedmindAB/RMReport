angular.module('webLog')
    ...
    $scope.createChart = function(data, id) {
    	...
    	$scope.homeData[id]=dataObject;
    	$scope.homeOptions[id]= $scope.getHomeOptions(dataObject.length);
    };
    ...
    .success(function(data, status, headers, config){ 
    	if(data){
    		...
    		for (var i = 0; i < $scope.allSuites.length; i++) {
				$scope.getSuiteByID($scope.allSuites[i].id);
			}
    	};
    }).error(function(data, status, headers, config){
    	console.log(data);
