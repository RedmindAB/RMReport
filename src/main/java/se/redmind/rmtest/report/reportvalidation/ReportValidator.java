...
import se.redmind.rmtest.db.create.gherkinstepinserter.GherkinStepInserter;
...
import se.redmind.rmtest.db.lookup.gherkin.GherkinStepDbLookup;
...
public class ReportValidator {
    ...
    private GherkinStepInserter gherkinStepInserter;
    ...
    public ReportValidator(String filename, String path) {
        ...
        gherkinStepInserter = new GherkinStepInserter();
    ...
    private void loadReport() {
        ...
        if (this.reportFile.getAbsolutePath().endsWith("xml")) {
            this.report = parser.getSimpleReportFromFile(reportFile).build();
        } else if (this.reportFile.getAbsolutePath().endsWith("json")) {
            this.report = new JsonReportBuilder(this.reportFile).build();
        }
    }
    ...
    public HashMap<String, Integer> getTestClassIDs(List<String> testClassNames) {
        HashMap<String, Integer> testClassIDs = new HashMap<>();
    ...
    private HashMap<String, Integer> getTestCases(Report report, HashMap<String, Integer> classIDs) {
        ...
        HashSet<String> addedTestCases = new HashSet<>();
    ...
    private void insertGherkinSteps(ReportTestCase testCase, Integer classID, String methodName, Report report) {
        GherkinStepDbLookup readStepsFromDB = new GherkinStepDbLookup();
        HashMap<String, Integer> allFromStepsConcat = readStepsFromDB.getAllFromStepsConcat();
        TestcaseDbLookup readTestcaseFromDB = new TestcaseDbLookup();
        int testcaseId = readTestcaseFromDB.getTestCaseID(methodName, classID.toString());
        if (!testCaseContainsKey(testCase.getGherkinSteps(), allFromStepsConcat, report, testcaseId)) {
            gherkinStepInserter.addStepsToBatch(testcaseId, testCase.getGherkinSteps(), report.getTimestamp());
            gherkinStepInserter.executeBatch();
        }
    }

    private boolean testCaseContainsKey(List<String> steps, HashMap<String, Integer> map, Report report, int
            testcaseID) {
        for (String step : steps) {
            String searchKey = step + testcaseID + report.getTimestamp();
